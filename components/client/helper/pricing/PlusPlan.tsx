...
import { PlanUserType, UserType } from '@/types/types';
...
export default function PlusPlan({
    ...
    return (
        <>
            <div
                className={`bg-white shadow-2xl w-full rounded-xl flex flex-col transition-all duration-300 border-2 border-indigo-500 scale-[1.02]`}
            >
                {/* Header Section */}
                <div className={`p-6 rounded-t-xl bg-indigo-50`}>
                    <div className="flex items-center justify-between">
                        <div>
                            <h2 className="font-bold text-2xl text-gray-900">Thành viên (Plus)</h2>
                            <p className="text-sm mt-1 text-gray-500">GoCart, Tư cách thành viên (Plus)</p>
                        </div>
                    ...
                    <div className="flex flex-col gap-1 mt-4">
                        <p className="text-4xl font-extrabold text-gray-900">
                            {thousandSeparator(192000)} {currency}
                        </p>
                        <span className="text-gray-500 text-base font-medium">/tháng (Chỉ thanh toán hàng tháng)</span>
                    </div>
                </div>
            ...
            {/* payment modal */}
            {paymentModal && <PaymentModal onClose={handleCloseModal} />}
        </>
    );
}
